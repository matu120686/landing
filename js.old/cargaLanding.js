...
/* ######################################################
  ...
  */
  ...
  /* Carga de los combo */
  $paisCombo = $('#pais')
  const url = '/rest/search/paises?oauth_token=8CDCD4D5-8284-48C0-B75A-4D3AAF379C87&limit=1000&sort=[{"property":o.pro_cdescripcion,"direction":ASC}]';
  ...
  $paisCombo.on('focus', function(){
      ...
      if ($paisComboValue != "") {
        const url = '/rest/t_provincias?oauth_token=8CDCD4D5-8284-48C0-B75A-4D3AAF379C87&limit=1000&filter=[{"property":o.pro_iParentID,"value":'+$paisComboValue+'}]&sort=[{"property":o.pro_cdescripcion,"direction":ASC}]&limit=9999';
      
        $.getJSON(url, function (data) {
            data.rows.forEach(function(row) {
                $ciudadCombo.append('<option value="'+ row.Id +'">'+ row.pro_cdescripcion +'</option>');
            })
        })
      }
  });
