   ...
   * En caso que no se encuentre cargada la variable principalServer
   */
    if (principalServer != "") {
        var urlfinal = principalServer
    } else {        
  ...
  function checkEnviar(event) {
    ...
    campo = $("#comentario");
    if (campo.val() != '') { campo.addClass("text_ok text-area"); comentario = campo.val(); } else {comentario = ""; }
    ...
    campo = $('#automonitoreo');
    if (campo.val() != '') { automonitoreo = campo.val().toLowerCase(); }

    campo = $('#diasprueba');
    if (campo.val() < 5 ) { diasprueba = 5; } else { diasprueba = campo.val(); }

    campo = $('#cuentatipo');
    if (campo.val() != '') { cuentatipo = campo.val(); } else { cuentatipo = ""; };

    var url = urlfinal+'/handler/landingKeyModulesCheck?oauth_token=8CDCD4D5-8284-48C0-B75A-4D3AAF379C87';
    if (error == 0) {
        //mando el formulario
        $.ajax({
            type: "POST",
